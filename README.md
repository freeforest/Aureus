...
Twelve Data Production data is session-only in V1. Search, Quote, OHLCV, and entitled Corporate Actions may use a future actor-owned 64 MiB transient memory store that does not serialize or survive App termination. Production persistent Twelve Data writes are **Disabled by product policy**; retention rights remain `BLOCKED`, but this deferred disk-cache question is no longer proposed as a permanent Stage 7 implementation blocker. Minimal user-authored symbol/MIC identifiers and UI preferences may persist without Provider descriptions or values. Basic US Search/OHLCV live acceptance and `XHKG`/`XSHG`/`XSHE`/`XJPX` remain `NOT VERIFIED`; no live-support claim is made.

The 2026-08-17 Terms refresh records §12.5 as immediate cessation of access with a deletion obligation and §16.2 as the operational deadline to delete within 30 days after termination or expiration. Aureus keeps immediate Provider-scoped purge as a stricter internal policy, not as a claimed Provider deadline. `/api_usage` remains `NOT VERIFIED`. No Provider request or Support message was performed in Stage 6L.
