...
Stages 1 through 5 are frozen after Reviewer Gate PASS. The current state is **Stage 6D Repair Candidate — Awaiting Reviewer Gate**. Wealth, Ledger, and Dashboard continue to use the Permanent Store. Stage 6 adds a Twelve Data `URLSession`/`Codable` adapter, native Keychain-only BYOK lifecycle, Frankfurter/ECB reference FX, a physically independent bounded Market Cache, typed TTL/LRU cleanup, offline/stale states, and minimal Provider/cache controls in Settings. Stage 6D makes same-Credential saves identity-preserving no-ops and stabilizes native Picker/Open/Save Panel automation without changing Production Ledger or CSV behavior. Markets remains an honest Stage 7 Placeholder.
