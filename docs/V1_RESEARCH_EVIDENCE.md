...
All prices and plan observations are point-in-time facts from 2026-08-10 and do not authorize a purchase. **U** means the required official fact was not established; **C** means official pages conflicted.
...
| Twelve Data | **V:** formal REST/WebSocket API and personal API key; no vendor SDK required. | Basic Free: 8 credits/minute, 800/day, three markets, batch supported, internal non-display, real-time US/FX/crypto and reference/technical data. | Grow from $29/month; Pro from $99/month (610+ API credits/minute); current page also displayed Pro 1597 at $229/month or $191/month equivalent annually. The four target regions require Pro according to the exchange list. | **REJECT as current Primary; paid conditional candidate** — Pro is not user-authorized and exact subscribed cache/display/export rights still require confirmation. |
...
| EODHD | **V:** formal keyed REST API; Search, EOD OHLCV, adjusted data and actions; no SDK required. | Free $0: 20 calls/day, 20 requests/minute, past-year range, personal use. One symbol normally costs one call; Search costs one; whole-exchange bulk costs 100. Exact four-market free entitlement is **U**. | EOD Historical Data — All World: $19.99/month or $199/year; 100,000 calls/day, 1,000 requests/minute, 30+ years, personal use. | **REJECT free; paid conditional candidate** — free quota is impractical; paid is unauthorized and exact HK/Tokyo EOD price coverage remains unresolved. |
...
| Twelve Data | V | V/P; span varies by plan/market | V/P splits, dividends, adjustments | V/P | Basic real-time US; EOD mainland China; exact HK/JP delay U | V Basic | P `XHKG` | P `XSHG`/`XSHE` | P `XJPX` |
...
| Twelve Data | Individual plans are personal/internal; each user can technically supply a separate key, but the applicable display entitlement depends on plan/use. | Terms permit only documented cache periods and require deletion after termination; exact subscribed period must be confirmed. | External display/redistribution/export requires the corresponding rights; official attribution guidance distinguishes internal/private from public display. | Technically plausible paid BYOK route, but no plan or rights are authorized. |
...
**Inference:** no free path passes both operational budget and the complete four-market/rights/cache constraints. Caching helps Twelve Data and FMP quota math but cannot create paid market entitlements; it cannot be used at all with Tiingo Starter's persistent-storage prohibition.
...
- Twelve Data: **REJECT as current Primary; retain as paid conditional candidate**.
- Marketstack: **REJECT**.
- Tiingo: **REJECT**.
...
- V1 Primary Market Data Provider: **BLOCKED — Result B; no Provider or plan selected**.
- Test/mock direction: **ACCEPT only as a future deterministic in-memory synthetic test provider conforming to the minimal contract; it is not a production Provider and does not authorize Stage 2.**

**Blocking basis:** [F] No free or practically usable free plan has a complete official evidence chain for all four regions, Search, historical OHLCV/adjustment semantics, sustainable quota, persistent local cache, retention/deletion, attribution, and personal BYOK desktop rights. [I] Freezing one would silently reduce scope or accept an unauthorized paid/legal assumption. A Provider-independent foundation does not mean Stage 1 passed and does not authorize Stage 2.

### 5.7 User Decision Packet

The Executor makes no selection. Only these evidence-backed decision boundaries can unblock A-008:

1. **Keep the complete US/HK/mainland-China/Japan scope and keep Production Provider work blocked.** No budget or scope assumption is made.
2. **User explicitly authorizes a named paid Provider and plan, followed by entitlement/rights confirmation before freeze.** The two strongest bounded candidates are:
   - **Twelve Data Pro 610, from $99/month:** official exchange evidence maps US plus `XHKG`, `XSHG`, `XSHE`, and `XJPX` to the Pro tier, with 610+ API credits/minute and no daily limit. Outstanding: exact purchased plan, EOD/delay freshness for HK/JP, cache window, termination deletion, internal-display/export/attribution rights, and BYOK open-source desktop interpretation.
   - **EODHD EOD Historical Data — All World, $19.99/month or $199/year:** official Pricing verifies 100,000 calls/day, 1,000 requests/minute, and 30+ years. Outstanding: authoritative EOD price coverage and symbol codes for Hong Kong and Tokyo (the stock-market list verifies US/Shanghai/Shenzhen but omits HK/Tokyo; `XHKG`/`XTKS` are verified only for the separate trading-hours endpoint), persistent-cache permission, termination deletion timing, exact personal local-display/BYOK interpretation, redistribution, and attribution. The direct Terms page was inaccessible in this execution, so those rights are not asserted.
   Listing these prices is not approval to purchase, create an account, or select either Provider. A two-Provider combination was considered but not frozen: it would still require paid authorization/rights confirmation and would add routing, provenance, cache, and failure complexity without an evidence-complete free solution.
3. **User explicitly approves a V1 market-coverage change.** The new covered/excluded markets and manual-import behavior must be stated as a product decision; the Executor cannot infer this from Provider limitations.

Candidate-to-source trace: Yahoo/yfinance M-01–M-03; Alpha Vantage M-04–M-07; Stooq M-08–M-09; Twelve Data M-10–M-15 and M-21; Marketstack M-16–M-20; Tiingo M-22–M-29; EODHD M-30–M-37; Finnhub M-38–M-41; FMP M-42–M-49. Each row in the source register carries the official URL, page-specific observation, verification state, and Aureus impact; all share the recorded 2026-08-10 visit date.

Official evidence: M-01 through M-49 in the [source register](#10-official-source-register).
...
| M-30 | [EODHD Pricing](https://eodhd.com/pricing) | Free 20 calls/day, past year; All World $19.99/month or $199/year, 100,000/day, 1,000/minute, 30+ years, personal use. | VERIFIED | Free budget fails larger sensitivity cases; paid authorization absent. |
...
| Primary Market Data Provider | **BLOCKED — Result B** | No free or practically usable free plan satisfies the combined four-market, data, quota, cache, deletion, attribution, and BYOK evidence. | One of the three user-owned choices in the User Decision Packet; then re-review A-008. |
...
| A-008 Market | M-01–M-49 | All nine required candidates were checked. Every free path fails at least one hard coverage/data/quota/cache/rights condition; paid paths are unauthorized and retain stated gaps. | **BLOCKED — Result B**; Provider-independent contract/test direction only, not Stage 1 PASS or Stage 2 authorization. |
...
This evidence supports a coherent architecture candidate except for A-008. It does not prove credentialed endpoint behavior, paid entitlement, provider SLA, production latency, signed sandbox behavior, build success, or test success. A-008's Provider-independent contract direction is not Stage 1 completion and does not authorize Stage 2. No unsupported market claim is converted into a product-scope reduction, paid authorization, or mock-as-production claim.
