...
| S11-10；A484/421，S129；Stage6/7→11 | DS.search/latestQuote/historicalBars/corporateActions四处fallback miss现已保留offline/timeout→既有MarketsFeatureModel.apply/disclosure；Model/View、FX及session helper未改 | DT.emptySessionErrors/staleSessionValues/corporatePairErrorMatrix/staleDoesNotHideOrdinaryErrors；MarketsTerminalTests.emptySessionSearchFailure/emptySessionHistoryFailure通过公开入口验证exact state/disclosure与无伪造数据；旧两处offline→missing预期相应改为offline | `PARTIAL / NOT VERIFIED`；源码修正已编译，E-MSO取消前尚未到达新增断言。实际offline UI/人工QA `NOT RUN` | D-05服务/Model验证尚未闭合；既有concurrencyAndCredits停滞后用户授权取消，根因UNKNOWN，不能宣称错误传播已获runtime PASS |
...
除已接受的D-02和已明确授权、但验证未闭合的D-05外，以下仍是**建议，未批准、未实施**。不得据此自行扩展范围或将Stage11缺口挪到Stage12。
...
| D-05 Offline empty-session呈现 | Reviewer已裁定四个Service入口保留原始offline/timeout；native missing不变，Model现有映射冻结 | 已仅修改DS及两个Unit文件；保留stale与完整action pair、FX和generation边界，不新增网络/磁盘fallback | E-MSO：unsigned BFT成功，Focused用户取消并完整Failed；新增Service/Model断言未到达，验证未闭合。实际离线UI/人工QA仍NOT RUN；不扩大授权修复既有并发问题 |
