...
**Status:** Stage 6 Repair Candidate — awaiting Reviewer Gate  
...
| TD-05 | [Credits](https://support.twelvedata.com/en/articles/5615854-credits) | 2026-08-13 | Credits are endpoint-weighted; quota is restored at the start of a new minute and Basic daily credits reset at UTC midnight. | VERIFIED | The gate uses exact epoch-minute and UTC-day boundaries; endpoint cost is checked before queuing; 429 preserves `Retry-After`. |
...
| Capabilities | Basic remains the US-focused Search/OHLCV path and does not advertise Corporate Actions. XNAS/XNYS and the small documented US MIC set aggregate to a conservative `US` entitlement while each instrument keeps its original MIC. XHKG/XSHG/XSHE/XJPX remain separate. Official catalog, live-observed, and not-verified evidence are distinct. | PASS — deterministic mapping tests; paid markets NOT VERIFIED |
| Request gate | Maximum two concurrent requests; Basic seeds 8 credits/minute and 800/day; exact minute-boundary and UTC-midnight resets; explicit pre-queue endpoint weights; unknown/untrusted plan text cannot raise limits; observed stricter limits apply. Identical concurrent GETs share one transport task; one waiter cancellation is isolated; last-waiter cancellation and Disconnect stop the underlying request and clear state. GET-only transient retry honors `Retry-After`, bounded backoff/jitter and cancellation. | PASS — deterministic clock/sleeper/transport tests |
...
| Market Cache | Independent GRDB cache v2, typed metadata/TTL, access tracking, 512 MiB default, 128 MiB–4 GiB range, 90% trigger, 80% target, cleanup ordering, LRU, expired removal, Provider purge, full own-database reset, and write refusal when the bound cannot be met. Legacy `cached_instruments`/`cached_prices` and current entries are all counted and are all eligible for typed expiry/LRU/capacity cleanup. Launch, periodic, and background schedules use matching independent metadata. | PASS — v1/current migration, mixed-capacity, scheduling and isolation tests |
...
| Settings | Native Provider/Keychain and cache controls; Missing/Invalid/Upgrade/Unsupported/Rate Limited states; plan metadata separated from observed entitlement; five-market and endpoint evidence status; explicit retention-blocked message; attribution, cache usage and destructive confirmations. Markets remains a Stage 7 Placeholder. | PASS — implementation, synthetic lifecycle UI automation, and read-only Production `Missing` observation |
