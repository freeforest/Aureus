...
**Status:** Stage 6B Repair Candidate — awaiting Reviewer Gate  
...
| Capabilities | Basic remains the US-focused Search/OHLCV catalog path and does not advertise Corporate Actions. Verified `/api_usage` plan state, official minimum-plan catalog evidence, endpoint-specific live observation, and market-specific live observation are independent. Search does not infer Quote/OHLCV market access; Quote/OHLCV preserve the successful raw MIC while the documented US MIC set can aggregate to `US`. Split and Dividend observations update independently. XHKG/XSHG/XSHE/XJPX remain separate. Session-open flags, interval, plan, and catalog do not manufacture freshness; absent explicit response evidence, freshness is `unknown`. | PASS — deterministic mapping tests; paid markets and actual freshness NOT VERIFIED |
| Request gate | Maximum two concurrent requests; Basic seeds 8 credits/minute and 800/day; exact minute-boundary and UTC-midnight resets; explicit pre-queue endpoint weights; unknown/untrusted plan text cannot raise limits; a verified paid usage response can explicitly represent no daily cap. Credits commit only after concurrency admission, so queue cancellation creates no false consumption; every delivered transport/retry attempt is counted. Identical concurrent GETs share one immutable request generation. Completion/cancellation/Disconnect callbacks must match both key and generation, so an old callback cannot remove or resume a newer same-key request. GET-only transient retry honors checked `Retry-After`, bounded backoff/jitter and cancellation. | PASS — deterministic clock/sleeper/generation-race/transport tests |
