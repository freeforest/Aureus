...
enum GoalValidationError: Error, Equatable, Sendable {
    case emptyName
...
struct Goal: Identifiable, Codable, Equatable, Sendable {
    ...
    init(
        ...
        name: String,
        target: Money,
        targetDate: CivilDate? = nil
    ) throws {
        let normalizedName = name.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalizedName.isEmpty else { throw GoalValidationError.emptyName }
        guard target.minorUnits > 0 else { throw GoalValidationError.nonPositiveTarget }
        self.id = id
    ...
    func validated() throws -> Goal {
        try Goal(id: id, name: name, target: target, targetDate: targetDate)
    }

    private enum CodingKeys: String, CodingKey {
        case id, name, target, targetDate
    }
    ...
    init(from decoder: any Decoder) throws {
        ...
        try self.init(
            ...
            target: container.decode(Money.self, forKey: .target),
            targetDate: container.decodeIfPresent(CivilDate.self, forKey: .targetDate)
        )
    }
