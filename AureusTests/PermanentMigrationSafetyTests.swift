...
struct PermanentMigrationSafetyTests {
    ...
    @Test("Recognized legacy v1 through v6 Backup exactly once before migration", arguments: [1, 2, 3, 4, 5, 6])
    ...
    func futureSchema() throws {
        ...
        try mutateMetadata(context.paths.permanentDatabaseURL) { db in
            try db.execute(
                sql: "UPDATE schema_metadata SET version = 8 WHERE store_kind = 'permanent'"
            )
        }
    ...
    func productionCompositionRootWiring() async throws {
        ...
        #expect(generation.manifest.schemaVersion == 1)
        #expect(try await dependencies.wealthStore.schemaVersion() == 7)
    }
    ...
    func concurrentConstruction() async throws {
        ...
        #expect(try migrationInventory(context).validGenerations.count == 1)
        #expect(try await stores[0].schemaVersion() == 7)
        #expect(try await stores[1].schemaVersion() == 7)
    }
    ...
    func sharedValidation() async throws {
        ...
        #expect(inspection.schemaVersion == 7)
