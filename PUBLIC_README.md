...
The current packaging name is **0.1 candidate**, retaining application version 0.1 / build 1. It is a review candidate, not a declaration of finished V1, Release Gate approval or public release.

The code input has accepted historical Full Unit evidence (497 definitions / 632 executions, no failures or skips). That does not verify this packaged application's launch or replace manual QA. Six controlled synthetic Search/History error modes have Model/Unit evidence only. Native failure observation, stale behavior, Cancel/Probe preservation, complete keyboard/VoiceOver and display-condition coverage, security-scope release counts, six performance protocols/measurements, and OSLog delivery/retention/manual redaction remain limited or unverified. Some synthetic backup and temporary-graph checks have bounded evidence; not every recovery scenario is accepted. Historical failed UI runs remain failed and are not combined with later individual passes.

No real financial records, credentials or Provider payloads are supplied. Keep private records and backups outside source trees and public/shared folders. Backups are **not independently encrypted by Aureus**; use private storage you control with appropriate system/storage encryption. Do not use this candidate as your only copy of important records.

Twelve Data V1 processing is session-only: persistent writes are Disabled and retention rights remain BLOCKED. The isolated permanent store and recoverable market cache retain separate capacity, TTL and cleanup rules; cleanup must not delete permanent wealth records. Production credentials are entered only by the user in native Settings and stored in the application's Keychain scope. Catalog visibility or synthetic success is not proof of the user's actual Plan or endpoint × MIC permission. This candidate makes no new live Provider or legal-rights claim.

## Install the candidate

The DMG contains a locally **ad-hoc signed** App. It has **no Developer ID signature and is not notarized by Apple**. A matching checksum shows file integrity relative to that checksum, not trusted publisher identity or freedom from malware. Obtain the candidate and its checksum from a source you trust.
...
2. Open `Aureus-0.1-candidate-macos-arm64.dmg`, review the installation and license information, then manually copy `Aureus.app` to your chosen application folder. Do not replace an existing App or private data without your own recovery plan. Eject the image before opening the installed copy.
...
Packaging, a read-only DMG mount and an install-directory copy are not an installed-App launch test. A separate attended, synthetic/temporary first-launch and basic-navigation check is still required before any launch-quality claim. A local unquarantined copy does not establish the behavior of a freshly downloaded copy.
...
Outputs in `Package/Artifacts` are `Aureus-0.1-candidate-macos-arm64.dmg`, `Aureus-0.1-candidate-source.zip`, and `SHA256SUMS`. Only these reviewed public artifacts are intended for handoff. `Package/Audit` contains local command paths and product records and is **not public material**. The script's checks do not replace review of the source whitelist or installed-App QA.
